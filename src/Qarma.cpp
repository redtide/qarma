    ...
    , m_type(Invalid)
{
    ...
    if (argList.at(0).endsWith("-askpass")) {
        argList.removeFirst();
        args << "--title" << tr("Enter Password") << "--password"
             << "--prompt" << argList.join(' ');
    } else {
...
static void addItems(QTreeWidget* tw, QStringList& values, bool editable,
    bool checkable, bool icons)
{
...
char Qarma::showList(const QStringList& args)
{
        ...
        if (args.at(i) == "--text") {
            ...
            if (needFilter) {
                ...
                connect(filter, &QLineEdit::textChanged, this,
                    [=](const QString& match) {
                        for (int i = 0; i < tw->topLevelItemCount(); ++i)
                            tw->topLevelItem(i)->setHidden(
                                !tw->topLevelItem(i)->text(0).contains(
                                    match, Qt::CaseInsensitive));
                    });
            }
    ...
    tw->setProperty(
        "qarma_list_flags", int(editable | checkable << 1 | icons << 2));
...
void Qarma::readStdIn()
{
    ...
    if (m_type == Progress) {
        ...
        if (QTreeWidget* tw = m_dialog->findChild<QTreeWidget*>()) {
            ...
            addItems(
                tw, input, twflags & 1, twflags & 1 << 1, twflags & 1 << 2);
        }
    }
    if (notifier)
        notifier->setEnabled(true);
    ...
    if (helpDict.isEmpty()) {
        ...
        helpDict["list"] = CategoryHelp(tr("List options"),
            HelpList()
                ...
                << Help("--mid-search",
                       tr("Change list default search function searching for"
                          "text in the middle, not on the beginning")));
        ...
        helpDict["password"] = CategoryHelp(tr("Password dialog options"),
            HelpList() << Help("--username", tr("Display the username option"))
                       << Help("--prompt=TEXT",
                              tr("The prompt for the user (QARMA ONLY!)")));
    ...
    if (category == "all") {
        foreach (const CategoryHelp& el, helpDict) {
            ...
            foreach (const Help& help, el.second)
                printf("  %-53s%s\n", qPrintable(help.first),
                    qPrintable(help.second));
            printf("\n");
        }
        return;
    }
